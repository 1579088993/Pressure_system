...
./src/time.c
# 汇编源文件
ASMS         = ./boot/startup_stm32f10x_md.s

# 包含路径
INCLUDES     = -I./boot/ -I./cmsis/ -I./inc/ -I./src/
# 宏定义
DEFINES      =
# 架构
CORE         = -mcpu=cortex-m3
...
CCFLAGS := $(CORE) -g -fomit-frame-pointer -fno-common -Wall -mthumb -mno-thumb-interwork\
           $(OPTIMIZATION) $(INCLUDES) $(DEFINES) -ffunction-sections -fdata-sections
# -ffunction-sections 每个函数单独成节
# -fdata-sections 每个数据单独成节

# 链接参数
LDFLAGS := -T $(LD_FILE) $(CORE) -Wl,-Map="$(OBJ_DIR)\$(TARGET).map" -nostartfiles -Wl,-gc-sections
